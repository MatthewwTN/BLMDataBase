 ...
 * they will all at least have a recordType (for example, "Online", "Book", 
"Movie"),
 * an identifier (like the name of the book, movie, or online organization or 
article),
 * and a description.
...
public abstract class Record implements CompareToByInterface
{
     ...
     * they will sometimes go out of the screen or wrap in ways that won't look 
    good.
     * This method solves that problem by returning a string representation of the 
    description 
     * that separates it onto different lines.
     * PRE: none
     * POST: class instance is unchanged. This returns a properly formatted String 
     ...
     * This method is useful because, the Record class implements the 
    CompareToByInterface, 
     * and the CompareToBy method might need to check if a Record has a particular 
    attribute
     * before it attempts to compare a Record using that attribute.
     * NOTE: the attribute parameter here is NOT case sensitive.  
     *       Thus, if the attribute parameter is "auTHOR", but the class has an 
    attribute called "author" or "Author",
    ...
    public boolean hasAttribute(String attribute){
        ...
        for (int i = 0; i < fields.length; i++){
            ...
            if 
            (fields[i].toString().toLowerCase().contains(attribute.toLowerCase()) ) {
